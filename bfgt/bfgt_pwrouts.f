...
c
C*********************************************************************C
      subroutine g3d_tens_prod_to_pw_old(nd,n,fvals,npw,
     1    tab_poly2pw,pwexp)
C*********************************************************************C
c     This routine computes the plane wave expansion from
...
c
C*********************************************************************C
      subroutine g3d_tens_prod_to_pw(nd,n,fvals,npw,
     1    tab_poly2pw,pwexp)
C*********************************************************************C
c     This routine computes the plane wave expansion from
c     tenosr product function values.
c     calls zgemv to compute the matrix-vector product
c
c     INPUT:
c     nd       vector length (for multiple RHS)
c     n        dimension of coeff array
...
c
      ...
      do ind = 1,nd
         ...
         do m2=1,n
         do m1=1,n
            cfvals(m1,m2,m3)=fvals(ind,m1,m2,m3)
         enddo
         enddo
         enddo
c        transform in z
         call zgemm('n','n',n*n,((npw+1)/2),n,alpha,cfvals,n*n,
     1       tab_poly2pw,n,beta,ff,n*n)
c
         do m1 = 1,n
         do k3 = 1,((npw+1)/2)
         do m2 = 1,n
         ...
         enddo
c        transform in y
         call zgemm('t','n',npw,((npw+1)/2)*n,n,alpha,
     1       tab_poly2pw,n,fft,n,
     2       beta,ff2,npw)
c        transform in x
         call zgemm('t','t',npw,((npw+1)/2)*npw,n,alpha,
     1       tab_poly2pw,n,ff2,npw*((npw+1)/2),
...
c
C*********************************************************************C
      subroutine g3d_pw2pot_old(nd,n,npw,pwexp,tab_pw2pot,pot)
C*********************************************************************C
c     This routine computes the potential on a tensor grid from 
c     the plane wave expansion coefficients (implicitly about the box center).
c
...
c        transform in x
     ...
     1       tab_pw2pot,npw,pwexp(1,1,1,ind),npw,beta,ff,n)
...
c
         ...
         enddo
c        transform in y
         call zgemm('t','n',n,n*((npw+1)/2),npw,alpha,
     1       tab_pw2pot,npw,fft,npw,beta,ff2t,n)
...
c        transform in z
     ...
     1       ff2,n*n,tab_pw2pot,npw,beta,zpot,n*n)
c
         do k3 = 1,n
         do k2 = 1,n
         do k1 = 1,n
            pot(ind,k1,k2,k3)=pot(ind,k1,k2,k3)
     1          +dreal(zpot(k1,k2,k3))*2
         enddo
         enddo
         enddo
c
...
c
C*********************************************************************C
      subroutine g3d_pw2pot_blas_slow(nd,n,npw,pwexp,tab_pw2pot,pot)
C*********************************************************************C
c     This routine computes the potential on a tensor grid from 
c     the plane wave expansion coefficients (implicitly about the box center).
c
...
c----------------------------------------------------------------------c
      ...
      complex *16 pwexp(npw,npw,((npw+1)/2),nd)
      complex *16 tab_pw2pot(npw,n)
      complex *16 tabz(((npw+1)/2),n)
      real *8 pot(nd,n,n,n)

      complex *16 ff(npw,npw,n)
      complex *16 fft(npw,n,npw)
      complex *16 ff2(n,n,npw)
      complex *16 zpot(n,n,n)
      complex *16 cd
      complex *16 alpha,beta
c
      ...
      do k=1,n
         do i=1,((npw+1)/2)
            tabz(i,k)=tab_pw2pot(i,k)
         enddo
      enddo
      
      do ind = 1,nd
c        transform in z
         call zgemm('n','n',npw*npw,n,((npw+1)/2),alpha,
     1       pwexp(1,1,1,ind),npw*npw,tabz,((npw+1)/2),beta,ff,npw*npw)
c
         do m1 = 1,npw
         do k3 = 1,n
         do m2 = 1,npw
         ...
         enddo
c        transform in y
         call zgemm('t','n',n,n*npw,npw,alpha,
     1       tab_pw2pot,npw,fft,npw,beta,ff2,n)
         
c        transform in x
         call zgemm('t','t',n,n*n,npw,alpha,
     1       tab_pw2pot,npw,ff2,n*n,beta,zpot,n)
c
         do k3 = 1,n
         do k2 = 1,n
         do k1 = 1,n
...
c      
c     BLAS does not have vector elementwise multiplication
c     level 2 zgbmv (i.e., treat one vector as a diagonal matrix) seems slower
c     than simple do loop!
c      
c      integer incx,incy
...
c      incy = 1
      ...
      enddo
c         call zgbmv('n',nexp,nexp,0,0,alpha,wshift,1,
c     1       pwexp1(1,ind),incx,beta,pwexp2(1,ind),incy)
      enddo
c
      return
      end
...
C
      ...
      enddo
c     Intel MKL subroutine for elementwise vector multiplication
c     slower than simple do loop
c         call vzmul(nexp,wshift,pwexp1(1,ind),pwexp2(1,ind))
      enddo
c
      return
      end
