...
c
      ...
      if (npwlevel.le.1 .and.iperiod.eq.1) then
         npwlevel=0
c         call prinf("npwlevel=1,but artificially set to 0*",i,0)
      endif
...
c            bsize=2*boxsize(max(npwlevel,0))
            bsize=boxsize(0)
...
c
      if (npwlevel.le.0.and.iperiod.eq.1) then
...
C$    time1=omp_get_wtime()
     ...
     4       ntarg,targs,ifpghtarg,pote,grade,hesse,timeinfo)
      ...
      call cpu_time(time2)
C$        time2=omp_get_wtime()
      if( ifprint .eq. 1 ) call prin2('time on box fgt main=*',
     1   time2-time1,1)
...
c     1d direct evaluation tables
      ...
      real *8 centertmp(10,10),dxyz(10)
      integer iimage(10)
...
c      
      ...
      if (npwlevel.eq.0 .and. iperiod0.eq.1) nlevend=-1
...
c           Check if the current box is a leaf box            
     ...
     1    prinf('number of direct evaluation source boxes=*',ndirect,1)
      ...
      if ((ifnewtree.eq.1).or.(ifpghtarg.gt.0)) then
...
c      if (nleafbox.eq.ndirect .and. sigma.le.2*eps) goto 3000
cccc      goto 3000
...
C$OMP$SCHEDULE(DYNAMIC)
     ...
     1               rmlexp(iaddr(2,jbox)),wpwmsshift(1,k,klev))
              enddo
           endif
        enddo
C$OMP END PARALLEL DO        
...
cc
      call cpu_time(time1)
C$    time1=omp_get_wtime()
      if (npwlevel.eq.0.and.iperiod.eq.0) goto 2500
      
      do 2000 ilev = 0,nlevend
C$OMP PARALLEL DO DEFAULT(SHARED)
C$OMP$PRIVATE(ibox,jbox,j,nl1,ixyz,jlev,bs)
C$OMP$PRIVATE(i,k,nimage,iimage,centertmp,iperiodtmp,bsh,dxyz)
C$OMP$SCHEDULE(DYNAMIC)  
         do ibox = itree(2*ilev+1),itree(2*ilev+2)
...
cccc       jbox is the source box
             ...
             bs = boxsize(jlev)
             if (iperiod.eq.1.and.jlev.eq.1) then
                iperiodtmp=0
                call gnd_find_loctab_ind(ndim,iperiodtmp,
     ...
     3              tab_loc(1,1,-nloctab,ilev),
     4              tabx_loc(1,1,-nloctab,ilev),
     5              tabxx_loc(1,1,-nloctab,ilev),
     6              ind_loc(1,1,-nloctab,ilev),
     5              ixyz)
                nimage=0
                bsh=bs*0.25d0
                do i=1,ndim
                   iimage(i)=0
                   dxyz(i)=centers(i,jbox)-centers(i,ibox)
                   if (abs(dxyz(i)).gt.bsh) iimage(i)=1
                   if (iimage(i).eq.1) nimage=nimage+1
                enddo
c add image boxes. There should be dimension independent way
c of doing this. For now, do it in the simplest way possible 
c for two and three dimensions!
                if (nimage.gt.0) then
c first image, always farthest from the original source box
                   do i=1,ndim
                      if (dxyz(i).lt.-bsh) then
                         centertmp(i,1)=centers(i,jbox)+bs0
                      elseif (dxyz(i).gt. bsh) then
                         centertmp(i,1)=centers(i,jbox)-bs0
                      else
                         centertmp(i,1)=centers(i,jbox)
                      endif
                   enddo
                   if (nimage.eq.2 .and.ndim.eq.2) then
                      centertmp(1,2)=centers(1,jbox)
                      centertmp(2,2)=centertmp(2,1)

                      centertmp(1,3)=centertmp(1,1)
                      centertmp(2,3)=centers(2,jbox)
                   endif

                   if (nimage.eq.2 .and.ndim.eq.3) then
                      if (iimage(1).eq.0) then
                         centertmp(1,2)=centers(1,jbox)
                         centertmp(1,3)=centers(1,jbox)

                         centertmp(2,2)=centers(2,jbox)
                         centertmp(3,2)=centertmp(3,1)

                         centertmp(2,3)=centertmp(2,1)
                         centertmp(3,3)=centers(3,jbox)
                      endif
                      if (iimage(2).eq.0) then
                         centertmp(2,2)=centers(2,jbox)
                         centertmp(2,3)=centers(2,jbox)
                      ...
                      if (iimage(3).eq.0) then
                         centertmp(3,2)=centers(3,jbox)
                         centertmp(3,3)=centers(3,jbox)
                   ...
                   if (nimage.eq.3 .and.ndim.eq.3) then
                      centertmp(1,2)=centertmp(1,1)
                      centertmp(2,2)=centers(2,jbox)
                      centertmp(3,2)=centers(3,jbox)
                      ...
                      centertmp(1,4)=centers(1,jbox)
                      centertmp(2,4)=centers(2,jbox)
                      centertmp(3,4)=centertmp(3,1)

                      centertmp(1,5)=centertmp(1,1)
                      centertmp(2,5)=centertmp(2,1)
                      centertmp(3,5)=centers(3,jbox)

                      centertmp(1,6)=centertmp(1,1)
                      centertmp(2,6)=centers(2,jbox)
                      centertmp(3,6)=centertmp(3,1)
                      
                      centertmp(1,7)=centers(1,jbox)
                      centertmp(2,7)=centertmp(2,1)
                      centertmp(3,7)=centertmp(3,1)
                   endif
                   
                   do k=1,2**nimage-1
                      call gnd_find_loctab_ind(ndim,iperiodtmp,
     1                    centers(1,ibox),centertmp(1,k),bs,bs0,
     2                    mrefinelev,ixyz)
                      call gnd_tens_prod_to_pghloc(ndim,nd,norder,
     1                    fvals(1,1,jbox),ifpgh,pot(1,1,ibox),
     2                    grad(1,1,1,ibox),hess(1,1,1,ibox),nloctab,
...
c
 2500 continue
      call cpu_time(time2)
C$    time2=omp_get_wtime()  
      timeinfo(7) = timeinfo(7)+time2-time1
...
c     also use asymptotic expansion to evaluate gradient and hessian
 ...
 3500 continue
      call cpu_time(time2)
C$    time2=omp_get_wtime()  
      timeinfo(8) = timeinfo(8)+time2-time1
      if (ifnewtree .eq.0) then
         goto 5000
      else
         goto 4000
...
c      call prin2('lp norm of the potential=*',rnorm,1)
...
c     scaled desired precision for error
      ...
      nboxes0 = itree(2*nlevels+2)
c      call prinf('before refinement, nboxes0=*',nboxes0,1)
...
c      call prinf('nrefine=*',nrefine,1)
c      call prinf('nmaxrefine=*',nmaxrefine,1)
c      call prinf('maxrefinelev=*',maxrefinelev,1)
...
c     call tree_reorg
 ...
 4600 continue
      ...
      call cpu_time(time2)
C$    time2=omp_get_wtime()  
      timeinfo(9) = timeinfo(9)+time2-time1
c      call prinf('laddr=*', itree(iptr(1)),2*(nlevels+1))
...
C$    time1=omp_get_wtime()
     ...
     4       ifpgh,pot,coefsp,grad,hess)
      ...
      call cpu_time(time2)
C$    time2=omp_get_wtime()  
      timeinfo(10) = timeinfo(10)+time2-time1
      if(ifprint.ge.1)
...
C$    time1=omp_get_wtime()
     ...
     4    itree(iptr(5)),itree(iptr(6)),itree(iptr(7)))
      call cpu_time(time2)
C$    time2=omp_get_wtime()  
      timeinfo(11) = timeinfo(11)+time2-time1
      nboxes0=itree(2*nlevels+2)
      if(ifprint .ge. 1)
...
c     using coefficients for potential only
 ...
 5500 continue
      call cpu_time(time2)
C$    time2=omp_get_wtime()  
      timeinfo(12) = timeinfo(12)+time2-time1
...
C$OMP END PARALLEL DO         
      ...
      do i = nlevstart+1,nlevels
         do ibox = laddr(1,i),laddr(2,i)
c          reuse the memory for the local PW expansion below the cutoff level
           iaddr(2,ibox) = iaddr(1,ibox)
        enddo
      enddo
c     
