...
c
      ...
      integer nchild(nbmax),ichild(2**ndim,nbmax),nnbors(nbmax)
      integer nbors(3**ndim,nbmax)
      integer isrcse(2,nbmax)
      integer laddrtail(2,0:nlmax)
      integer, allocatable :: iflag(:)

      integer i,j,k,ibox,jbox,kbox,ilev,idad,igranddad
      integer nbloc,ict,mc,mnbors,ifnbor,npts,nlevend
      double precision dis,distest

      if (nlevels.lt.nlmax) boxsize(nlevels+1)=boxsize(nlevels)/2
...
C$OMP END PARALLEL DO     
      ...
      if (nlevels.lt.nlmax) then
         nlevend=nlevels
      elseif (nlevels.eq.nlmax) then
         nlevend=nlevels-1
      endif
...
C$OMP END PARALLEL DO      
     ...
     1    ndiv,nlmin,nlmax,ifunif,iper,
     2    bs0,cen0,nlevels,nboxes,ltree)
c
c
c
c----------------------------------------
...
c
     ...
     2    bs0,cen0,ltree,itree,iptr,centers,
     3    boxsize)
c
c
c
...
C$OMP END PARALLEL DO      
     ...
     2       itree(iptr(2)),itree(iptr(3)),itree(iptr(4)),
     3       itree(iptr(5)),itree(iptr(6)),itree(iptr(7)),isrcse)
