...
c
c
      subroutine ortho_trans_3d_old(nd,itype,norder,fin,fout,umat)
c
c     this subroutine transform the input data given on a tensor product grid
c     to the output data. Depending on the 1d transformation matrix umat, it can
c     be used in the following several cases. 
...
c
c
      subroutine ortho_trans_3d(nd,itype,n,fin,fout,umat)
c
c     this subroutine transform the input data given on a tensor product grid
c     to the output data. Depending on the 1d transformation matrix umat, it can
c     be used in the following several cases. 
...
c                         val2coefs or coef2vals
c      
c     n - order of the expansion in each direction
c     fin - input data
c     umat - 1D transformation matrix along each direction
c      
c     output:
...
c           transform in z
     ...
     1          fv,n*n,umat(1,1,3),n,beta,ff,n*n)
...
c           transform in y
     ...
     1          umat(1,1,2),n,fft,n,beta,ff2,n)

c           transform in x
            call dgemm('n','t',n,n*n,n,alpha,
     1          umat,n,ff2,n*n,beta,fo,n)
         
            do k3=1,n
            do k2=1,n
            do k1=1,n
               fout(ind,k1,k2,k3)=fo(k1,k2,k3)
            enddo
            enddo
            enddo
