...
c
     ...
     2     nboxes, mboxes, fevaln, tt, iperiod, eps, alpha,
     3     vpot, unm1, fnm2, fnm1, un, rhsfun1, rhsfun2,
     4     iadap)
      implicit real*8 (a-h,o-z)
c     global vars
      integer norder, ltree, nboxes, ndim, nlevels
...
c     complex allocatables
      complex*16, allocatable:: zpars(:)
      external fevaln, rhsfun1, rhsfun2
...
c             a leaf node
c            do i=1, npbox
c             write(*,*)ibox,i,un(1,i,ibox),unm1(1,i,ibox),
c     1           fnm2(1,i,ibox), fnm1(1,i,ibox)
c            enddo
c          endif
c        enddo
c      enddo
...
c     global vars
      ...
      real*8 targs(ndim,ntarg), eps, dt
      real*8 fnm2(norder**ndim,mxboxes)
      real*8 fnm1(norder**ndim,mxboxes)
...
c
      ...
      allocate(fn(npbox,mxboxes))
      allocate(vpot(npbox,mxboxes))
      allocate(vpotinc(npbox,mxboxes))
      allocate(vpotpinc(ntarg))
c
      allocate(xref(ndim,npbox))
      allocate(wts(npbox))
...
c
      if(iadap .gt. 0 ) then
        write(*,*) ' '
        write(*,*) 'calling am24_adap_tree'
c
c
      call am24_adap_tree(norder, nordert, ltree, itree,
     1     iptr,rintl, centers, nlevels,mxlevels, boxsize, nboxes,
     2     mxboxes,fevaln, tt, iperiod, eps,alpha,
     3     vpot,unm1,fnm2, fnm1,un, rhsfun1, rhsfun2,iadap)
...
c     mboxes and mlevels added
c
c     the diffusion consts: visc(nd)
c
c**************************************************************
c
      subroutine vpot_readiff_am2(nd, norder, nordert, ltree, 
     1       itree, iptr, centers, nlevels, mlevels, boxsize, 
     2       nboxes, mboxes, rintl, fevaln, devaln, tpre, dt, 
     3       iperiod, ntarg, targs, ifvtarg, ifptarg, visc, 
     4       iadap, eps, un)
      implicit real*8 (a-h,o-z)
c     global vars
      integer nd, norder, ltree, nboxes, ndim, nlevels
      integer iperiod, ntarg, mboxes, mlevels
      integer ifvtarg, ifptarg
      parameter(ndim=2)
      integer itree(ltree), iptr(12)
      real*8 centers(ndim, mboxes), boxsize(0:mlevels)
      real*8 targs(ndim,ntarg), eps, dt, visc(nd)
      real*8 un(nd,norder**ndim,mboxes)
      real *8 rintl(0:mlevels)
c     local vars      
      integer, allocatable:: ipars(:)
...
c
      allocate(tnodes(nordert))
      allocate(twhts(nordert))
c     be careful: nd-vecs, except for fright
c     since the FGTs should be called one by one
      allocate(fright(npbox,mboxes))
      allocate(unm1(nd,npbox,mboxes))
      allocate(fn(nd,npbox,mboxes))
...
c
      ...
      allocate(g(nd))
c
c      write(*,*) nd, norder, nordert, ltree
c      write(*,*) nlevels, mlevels, nboxes, mboxes
c      write(*,*) tpre, dt
c      write(*,*) iperiod, ntarg, ifvtarg, ifptarg
c      write(*,*) iadap, eps
c
c     initialize the grid and stuff
      itype = 0
      ipoly=1
      type='f'
      call polytens_exps_nd(ndim,ipoly,itype,norder,type,xref,
...
c--------------------------------------------------
c     contribution from t(jj): an FGT from last step
c     attention: since the diffusion consts are different
c     this should be nd- FGT calls
      scal=dt
      do jj=1,nnodes-1
c       loop over nnodes-1 nodes (in time)
        tt=tpre+tnodes(jj)
c        write(*,*) tpre, tnodes(jj), tt
ccc        be careful: the variances should be different 
ccc        for each FGT call
ccc        delta=(norder-1)*dt-tnodes(jj)
c        write(*,*) 'jj, nnodes', jj, nnodes
c
        if(jj .eq. nnodes-3) then
        elseif(jj .eq. nnodes-2) then
...
c             get the grid and eval the forcing term (using un)
                do j=1, npbox
                  do k=1, ndim
                    xtarg(k)=centers(k,ibox) + xref(k,j)*bs
                  enddo
c                 be careful about the interface of fevaln
ccc               be careful !!
c                  call fevaln(nd, tt,xtarg(1),xtarg(2),
c     1                 un(1,j,ibox),fn(1,j,ibox))
                   dpars(1)=visc(1)
                   dpars(2)=visc(2)
                   call fevaln(nd, tt, xtarg, un(1,j,ibox),
     1                  dpars, zpars, ipars, fn(1,j,ibox)) 
...
c
c       adding the contributions from un and fn
c       (initpot + volpot)
c       again do it component-wise
        do j=1, nd
cc        for each component of the system  
          do i=1, nboxes
            do k=1, npbox
              fright(k,i)=fn(j,k,i)*scal*twhts(jj)+un(j,k,i)
c              if(itree(iptr(4)+i-1).eq.0) then
c                write(501,*) fn(j,k,i), un(j,k,i), scal, twhts(jj), 
c     1                       k, i, j
c               write(501,*) fright(k,i), k,i,j
c              endif
            enddo
          enddo
c         fright correctly assigned
c
          do i=1, nboxes
            do k=1, npbox
              vpotinc(k,i)=0.0d0
...
c         remember to scale
c
c         attention: visc(j) multiplied here
c         it would be helpful to be able to deal with 
c         vectors of visc in boxfgt though 
          delta=4.0d0*(dt-tnodes(jj))*visc(j)
          ifnewtree = 0
          nd0 = 1

c
          t1=second()
          call boxfgt(nd0,ndim,delta,eps,ipoly,iperiod,norder,npbox,
     1         nboxes,nlevels,ltree,itree,iptr,centers,boxsize,
     2         fright,ifpgh,vpotinc,grad,hess,ifnewtree,ntarg,targs,
     3         ifpghtarg,vpotpinc,gradp,hessp,timeinfo)
          t2=second()
c
        write(368,*)'time for fgt=',t2-t1
        write(368,*)'speed for fgt=',npbox*nboxes/(t2-t1)
c
c         is vpot initialized properly though
          do i=1, nboxes
            if(itree(iptr(4)+i-1).eq.0) then
              do k=1, npbox
                 vpotinc(k,i)=vpotinc(k,i)/(pi*delta)
                 vpot(j,k,i)=vpot(j,k,i)+vpotinc(k,i)
c                 write(203,*) vpot(j,k,i), fright(k,i), j, k, i 
              enddo
            endif
          enddo
c       vpot(j,:,:) updated for all the jj-time nodes
c       except for the last one where we need a nonlinear solve
c
c       end of the component-wise j-loop
        enddo
c
c     end of the jj-do
      enddo
...
c
              ...
              enddo
c
c              write(512,*) u0(1), g(1), j, ibox
c              write(522,*) u0(2), g(2), j, ibox
c
              dpars(1)=visc(1)
              dpars(2)=visc(2)
              call newton(nd, ndim, fevaln, devaln, u0, 
     1             tt, xtarg, g, alpha, ipars, dpars, zpars,
     2             rtol, atol, kmax, u1, f1, res, ktot, 
     3             ierr, istat)
c
c              write(123,*) j,ibox,ierr,istat,ktot,res
c
              do ii=1, nd
                 un(ii,j,ibox)=u1(ii)
c                 write(124,*) un(ii,j,ibox), ii,j,ibox
              enddo
c
              if(ierr .gt. 0) ierrr = 1
c
c           end of the j loop
            enddo
c
...
c
        ...
        write(368,*)'time for solving nonlinear eq=',t4-t3
        write(368,*)'speed for solving nonlinear eq=',
     1               npbox*nboxes/(t4-t3)
...
c
c---------------------------------------------------
c     for tests only, turn it off
c      iadap = 0     
c
      if(iadap .gt. 0) then
        write(*,*) ' '
        write(*,*) 'calling am2_adap_tree_nd'

        t5=second()
        call am2_adap_tree_nd(nd, norder, ltree, itree,
     1       iptr, rintl, centers, nlevels, mlevels, boxsize, 
     2       nboxes, mboxes, fevaln,devaln, dpars, zpars, ipars,
     3       tt, iperiod, eps, alpha,
     4       vpot, un, iadap)
         t6=second()
c

        write(368,*)'time for adap=',t6-t5
        write(368,*)'speed for adap=',
     1               npbox*nboxes/(t6-t5)
...
c----------------------------------------------
c     subroutine am2_adap_tree_nd
c----------------------------------------------
c
c     2nd order version of am24_adap_tree_nd
c     which checks the resolution of the grid
c     after solving the nonlinear eqn 
c
c     the interface is more or less the same
c     as that of am24_adap_tree_nd
c     except that it's 2nd order only and 
c     needs to resolve fewer arrays
c
c     iadap = 0: no refinement or coarsening
...
c
c----------------------------------------------
c     need to modify the interface of newton
c
      subroutine am2_adap_tree_nd(nd, norder, ltree, itree,
     1     iptr, rintl, centers, nlevels, mlevels, boxsize, 
     2     nboxes, mboxes, fevaln, devaln, dpars, zpars, ipars,
     3     tt, iperiod, eps, alpha, vpot, un, iadap)
      implicit real*8 (a-h,o-z)
c     global vars
      integer norder, ltree, nboxes, ndim, nlevels
      integer mboxes, mlevels, nd
      integer iperiod, ntarg, ipars(100)
      parameter(ndim=2)
      integer itree(ltree), iptr(12)
      real*8 centers(ndim, mboxes), boxsize(0:mlevels)
      real*8 tt, eps, alpha, f0(1)
...
c      t2=second()
c
c      write(*,*)'speed for initialization',(t2-t1)
        ...
        do ibox = itree(2*ilev+1), itree(2*ilev+2)
          ...
          if(nchild .eq. 0) then
            do i=1,npbox
              ...
              call fevaln(nd, tt, xy, un(1,i,ibox),
     1             dpars, zpars, ipars, fn(1,i,ibox)) 
            enddo
          endif
        enddo
      enddo
...
c      t2=second()
c
c      write(*,*)'speed for eval fun',(t2-t1)
c
c--------------------------------------------------
c     convert potential to its polynomial expansion coefficients
c     (for all the leaf nodes)
...
c      t2=second()
c      write(*,*)'speed for convert potential',(t2-t1)
...
c              g = vpot(1,ii,jbox)
c              u0 = unm1(1,ii,jbox)
c              u1 = g+alpha*fn(1,ii,jbox)
c              tol = 1.0d-14
c             fix a problem: u0 and u1 are too close
c              if(abs(u0-u1) .lt. 10.0d0*tol) then
c                u1 = u0+10.0d0*tol
c              endif
c              call secant(fevaln,tt,xy(1),xy(2),u0,u1,g,alpha,u,fu,tol,
c     1             msi,isi,ndamp,ier)
c----------------------
              atol=1.0d-12
              rtol=1.0d-12
              kmax=100
              do kk=1, nd
                u0(kk)=un(kk,ii,jbox)
                g(kk)=vpot(kk,ii,jbox)
              enddo
              call newton(nd, ndim, fevaln, devaln, u0, 
     1             tt, xy, g, alpha, ipars, dpars, zpars,
     2             rtol, atol, kmax, u1, f1, res, ktot, 
     3             ierr, istat)
c
c             I suppose the indices are correct?
...
c
            if(erri .gt. reps*rintl(jlev)) then
c             flag the new box
c              write(*,*)jbox,erri,reps*rintl(jlev)
              ifrefine(jbox) = 1
            endif

          enddo
c         write(*,*)'11111111111111'
c       end refining box ibox
        endif
      enddo
      ...
      do ibox = 1, nboxes
        do i=1, npbox
          do kk=1, nd
            ...
            uallvals(nd+kk,i,ibox) = fn(kk,i,ibox)
c
            uallcoefs(kk,i,ibox) = ucoefs(kk,i,ibox)
            uallcoefs(nd+kk,i,ibox) = fcoefs(kk,i,ibox)
          enddo
        enddo
      enddo
c
...
c
c      t2=second()
c      write(*,*)'speed for coarse',(t2-t1)
      ...
      do ibox = 1, nboxes
        do i=1, npbox
          do kk=1, nd
            ...
            fn(kk,i,ibox) = uallvals(nd+kk,i,ibox)
          enddo
        enddo
      enddo
c
c      t4=second()
c      write(*,*)'speed for level-restricted',t4-t1
c
c------------------------------------------------
c
      end subroutine
...
c**************************************************************
c     subroutine vpot_readiff_am24
c**************************************************************
c
c     This subroutine computes the volume potential
c     by 2nd and 4th order Adams-Moulton methods
...
c**************************************************************
c
      subroutine vpot_readiff_am24(nd, norder, nordert, ltree,
     1       itree, iptr, centers, nlevels, mlevels, boxsize,
     2       nboxes, mboxes, rintl, fevaln, devaln, tpre, dt,
     3       iperiod, ntarg, targs, ifvtarg, ifptarg, visc,
...
c
          do i=1,nboxes
            do k=1,npbox
              fright(k,i)=fnm2(j,k,i)*scal*twhts(jj)
            enddo
          enddo
...
c
          do i=1,nboxes
            do k=1,npbox
              fright(k,i)=fnm1(j,k,i)*scal*twhts(jj)
            enddo
          enddo
        elseif(jj .eq. nnodes-1) then
c         evaluate fevaln at the grid pts
...
c             get the grid and eval the forcing term (using un)
                do l=1, npbox
                  do k=1, ndim
                    xtarg(k)=centers(k,ibox) + xref(k,l)*bs
                  enddo

                   dpars(1)=visc(1)
                   dpars(2)=visc(2)
                   call fevaln(nd, tt, xtarg, un(1,l,ibox),
     1                  dpars, zpars, ipars, fn(1,l,ibox))
...
c         it would be helpful to be able to deal with
c         vectors of visc in boxfgt though
          delta=4.0d0*((nordert-1)*dt-tnodes(jj))*visc(j)
          ifnewtree = 0
          nd0 = 1
...
c
      if(iadap .gt. 0) then
        write(*,*) ' '
        write(*,*) 'calling am2_adap_tree_nd'
c       I believe this is enough since it's all nd-veced
c       go over it again
c
c      write(*,*) npols, npbox, ndim, nd,tt, iperiod, eps
c      write(*,*)ltree,nlevels, mlevels,nboxes, mboxes
     ...
     2       nboxes, mboxes, fevaln,devaln, dpars, zpars, ipars,
     3       tt, iperiod, eps, alpha,
     4       vpot,fnm2, fnm1, unm1, un, iadap)
         t6=second()
c
...
c----------------------------------------------
c     subroutine am24_adap_tree_nd
c----------------------------------------------
c
c     2nd and 4th order version of am24_adap_tree_nd
c     which checks the resolution of the grid
c     after solving the nonlinear eqn
c
c     the interface is more or less the same
        ...
        do ibox = itree(2*ilev+1), itree(2*ilev+2)
          ...
          if(nchild .eq. 0) then
            do i=1,npbox
              ...
              enddo
c             attention: check the calling sequence
c              call fevaln(tt,xy(1),xy(2),un(1,i,ibox),
c     1             fn(1,i,ibox))
...
c            write(224,*) erra, errb, reps*rintl(jlev), ifrefine(jbox)
c           still the problem of rintl
ccc         refine a few more boxes to trigger coarsening
c            if(jbox .eq. 325) then
c              ifrefine(jbox)=1
      ...
      do ibox = 1, nboxes
        do i=1, npbox
          do kk=1, nd
            ...
            vpot(kk,i,ibox) = uallvals(8+kk,i,ibox)
          enddo
        enddo
      enddo
...
c**************************************************************
c
      subroutine vpot_readiff_am6(nd, norder, nordert, ltree,
     1       itree, iptr, centers, nlevels, mlevels, boxsize,
     2       nboxes, mboxes, rintl, fevaln, devaln, tpre, dt,
     3       iperiod, ntarg, targs, ifvtarg, ifptarg, visc,
...
c     global vars
      ...
      real*8 centers(ndim, mboxes), boxsize(0:mlevels)
      real*8 targs(ndim,ntarg), eps, dt, visc(nd)
      real*8 fnm4(nd,norder**ndim,mboxes)
...
c
      elseif(nordert .eq. 5) then
        ...
        tnodes(4)=3*dt
        tnodes(5)=4*dt
        twhts(1)= -19.0d0/720.0d0
        twhts(2)=106.0d0/720.0d0
        twhts(3)=-264.0d0/720.0d0
        twhts(4)= 646.0d0/720.0d0
        twhts(5)= 251.0d0/720.0d0
...
c
      elseif(nordert .eq. 6) then
        ...
        tnodes(5)=4*dt
        tnodes(6)=5*dt
        twhts(1)= 27.0d0/1440.0d0
        twhts(2)= -173.0d0/1440.0d0
        twhts(3)= 482.0d0/1440.0d0
        twhts(4)= -798.0d0/1440.0d0
        twhts(5)= 1427.0d0/1440.0d0
        twhts(6)= 475.0d0/1440.0d0
...
c
        if(jj .eq. nnodes-5) then
c
          do i=1,nboxes
            do k=1,npbox
              fright(k,i)=fnm4(j,k,i)*scal*twhts(jj)
            enddo
          enddo

        elseif(jj .eq. nnodes-4) then
c
          do i=1,nboxes
            do k=1,npbox
              fright(k,i)=fnm3(j,k,i)*scal*twhts(jj)
            enddo
          enddo
c
        elseif(jj .eq. nnodes-3) then
c
          do i=1,nboxes
            do k=1,npbox
              fright(k,i)=fnm2(j,k,i)*scal*twhts(jj)
...
c     update fnm4, fnm3, fnm2, fnm1 for higher order
c
      if(nordert .ge. 6) then
        do i=1, nboxes
          if(itree(iptr(4)+i-1).eq.0) then
            do k=1, npbox
              do j=1, nd
              fnm4(j,k,i)=fnm3(j,k,i)
              enddo
            enddo
          endif
        enddo
...
c      write(*,*)ltree,nlevels, mlevels,nboxes, mboxes
        ...
        call am6_adap_tree_nd(nd, norder, ltree, itree,
     1       iptr, rintl, centers, nlevels, mlevels, boxsize,
     2       nboxes, mboxes, fevaln,devaln, dpars, zpars, ipars,
     3       tt, iperiod, eps, alpha,
     4       vpot, fnm4, fnm3, fnm2, fnm1, unm1, un, iadap)
         t6=second()
c
...
c     need to modify the interface of newton
c
      subroutine am6_adap_tree_nd(nd, norder, ltree, itree,
     1     iptr, rintl, centers, nlevels, mlevels, boxsize,
     2     nboxes, mboxes, fevaln, devaln, dpars, zpars, ipars,
     3     tt, iperiod, eps, alpha,
     4     vpot, fnm4, fnm3, fnm2, fnm1, unm1, un, iadap)
...
c     need more of these fvals thing?
      ...
      real *8, allocatable :: grid(:,:),ximat(:,:),qwts(:)
      real *8, allocatable :: rmask(:), umat_nd(:,:,:)
      real*8, allocatable :: fcoefs3(:,:,:), fcoefs4(:,:,:)
      real *8 rintl(0:mlevels)
      real *8 xy(ndim)
      real*8, allocatable:: grad(:,:,:,:)
      real*8, allocatable:: hess(:,:,:,:)
...
c     un, fn, unm1, vpot
      ...
      allocate(fcoefs4(nd,npbox,mboxes))
c
      allocate(xq(norder),wts(norder))
      allocate(umat(norder,norder),vmat(norder,norder))
      allocate(umat_nd(norder,norder,ndim))
...
c
      call treedata_trans_nd(ndim,nd,itype,nlevels,itree,
     1    iptr,boxsize,norder,fnm4,fcoefs4,umat_nd)
c
      call treedata_trans_nd(ndim,nd,itype,nlevels,itree,
     1    iptr,boxsize,norder,un,ucoefs,umat_nd)
c
...
c
c           interpolate fnm4 too
c           for subsequent steps
            call ortho_eval_nd(ndim,nd,norder,
     1           fcoefs4(1,1,ibox),norder,fnm4(1,1,jbox),
     2           polyvc(1,1,1,j))
c
            call ortho_trans_nd(ndim,nd,0,norder,fnm4(1,1,jbox),
     1           fcoefs4(1,1,jbox),umat_nd)
c
c           now get the grid on the box jbox
c           and do the nonlinear solve again for un
            do ii=1, npbox
      ...
      do ibox = 1, nboxes
        do i=1, npbox
          do kk=1, nd
            ...
            uallvals(6+kk,i,ibox) = fnm2(kk,i,ibox)
            uallvals(8+kk,i,ibox) = vpot(kk,i,ibox)
            uallvals(10+kk,i,ibox) = fnm3(kk,i,ibox)
            uallvals(12+kk,i,ibox) = fnm4(kk,i,ibox)
c
          uallcoefs(kk,i,ibox) = ucoefs(kk,i,ibox)
          uallcoefs(2+kk,i,ibox) = ucoefs1(kk,i,ibox)
          uallcoefs(4+kk,i,ibox) = fcoefs1(kk,i,ibox)
          uallcoefs(6+kk,i,ibox) = fcoefs2(kk,i,ibox)
          uallcoefs(8+kk,i,ibox) = vcoefs(kk,i,ibox)
          uallcoefs(10+kk,i,ibox) = fcoefs3(kk,i,ibox)
          uallcoefs(12+kk,i,ibox) = fcoefs4(kk,i,ibox)
          enddo
        enddo
      enddo
c
      ...
      do ibox = 1, nboxes
        do i=1, npbox
          do kk=1, nd
            ...
            fnm2(kk,i,ibox) = uallvals(6+kk,i,ibox)
            vpot(kk,i,ibox) = uallvals(8+kk,i,ibox)
            fnm3(kk,i,ibox) = uallvals(10+kk,i,ibox)
            fnm4(kk,i,ibox) = uallvals(12+kk,i,ibox)
          enddo
        enddo
      enddo
