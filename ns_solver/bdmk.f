...
c     for the log kernel, there is a nonzero constant mode
      if (ndim.eq.2 .and. ikernel.eq.1) then
         ...
         enddo
c         call prin2('c0=*',c0,1)
      endif
      
c      call prin2('r0=*',r0,1)
c      call prin2('deltas=*',deltas,ng)
c      call prin2('ws=*',ws,ng)
c      call prin2('boxsize=*',boxsize,nlevels+1)
      ...
      enddo
c      call prinf('npwlevels=*',npwlevels,ng)
c      call prinf('ipw=*',ipw,nlevels-nlevstart+2)
c      call prinf('ipwaddr=*',ipwaddr,2*(nlevels-nlevstart+2))
...
c      call prin2('after substracting gaussian contri, c=*',c,ntaylor+1)
...
c         call prin2('the integral of the rhs=*',fint,nd)
