...
c
      call pts_tree_refine_once_build(dim,sources,ns,targ,nt,idivflag,
     1    ndiv,nlmin,nlmax,ifunif,iperiod,nlevels,nboxes,
     2    bs0,cen0,
...
c
      ...
      integer, allocatable :: ifpwexpform(:),ifpwexpeval(:)

      real *8, allocatable :: csourcesort(:,:),ctargetsort(:,:)
...
c     or this is if you want default opts, make a null pointer...
      ...
      integer dad,ilev,ibox,jbox,istart,iend,jstart,jend,isep
      integer istartt,iendt,jstartt,jendt
      integer istarts,iends,jstarts,jends
      integer npts, nptssrc,nptstarg,i,j,k,ind,nptstmp
      integer nexp,nhess,nmax,n1,nb,ns,nt,iperiod0,nleafbox
      integer mnlist1,nlevend,ier,mnlistpw,mnbors,mc
      real *8 time1,time2,xmin,d,dmax,delta0,bs0,hpw
...
c     written in C++
      allocate(ctargetsort(ntarget,dim))
      allocate(csourcesort(nsource,dim))

      do i=1,dim
         do j=1,ntarget
            ctargetsort(j,i)=targetsort(i,j)
         enddo
      enddo
...
c      
      ...
      call pfgt_find_pwexp_boxes_new(dim,ncutoff,nboxes,
     1    nlevels,ltree,itree,iptr,
     2    ndiv,nboxsrcpts,nboxtargpts,
     3    ifpwexpform,ifpwexpeval)
cccc      if (ifprint.eq.1) call prinf('ifpwexpform=*',ifpwexpform,nboxes)
cccc      if (ifprint.eq.1) call prinf('ifpwexpeval=*',ifpwexpeval,nboxes)
...
c     listpw contains source boxes in the pw interaction
      call pfgt_compute_listpw_new(dim,ncutoff,nboxes,nlevels,
...
cccc      if (ifprint.eq.1) call prinf('listpw=*',listpw,nboxes*mnlistpw)
      ...
      nlevend=nlevels
      if (npwlevel.lt.nlevels) nlevend=ncutoff+1
c
c     compute list info
c
      isep = 1
      call compute_mnlist1_new(dim,nboxes,nlevels,itree(iptr(1)),
...
c     at or above npwlevel
      call pfgt_compute_modified_list1_new(dim,
...
c        need better memory management or switch to a better language
         if (ifpwexpeval(ibox).eq.1 .and. ifpwexpform(ibox).eq.0) then
            call pfgt_pwexp_zero(nd,nexp,rmlexp(iaddr(2,ibox)))
         endif
      enddo
      enddo
...
c     direct evaluation if the cutoff level is >= the maximum level
      if (npwlevel .gt. nlevels) goto 1800
c      
c     get planewave nodes and weights
...
cccc              ibox is the target box
                  ...
                  if (nptstarg.gt.0.and.ifpghtarg.gt.0) then
                     call pfgt_direct_c(nd,dim,eps,delta,dmax,iperiod0,
     1                   jstart,jend,sourcesort,
     2                   ifcharge,chargesort,
     3                   ifdipole,rnormalsort,dipstrsort,
     4                   istartt,iendt,ntarget,ctargetsort,
     5                   ifpghtarg,pottarg,gradtarg,hesstarg)
                  endif
                  if (nptssrc.gt.0 .and.ifpgh.gt.0) then
c                     call pfgt_direct_c(nd,dim,eps,delta,dmax,iperiod0,
c     1                   jstart,jend,sourcesort,
c     2                   ifcharge,chargesort,
c     3                   ifdipole,rnormalsort,dipstrsort,
c     4                   istarts,iends,nsource,csourcesort,
c     5                   ifpgh,pot,grad,hess)
...
C$OMP END PARALLEL DO         
 2000 continue
      print *, '# of direct source boxes=', nb
      call cpu_time(time2)
C$    time2=omp_get_wtime()  
...
c
c------------------------------------------------------------------     
      subroutine pfgt_direct_c(nd,dim,eps,delta,dmax,iperiod,
...
c
      ...
      if ((ifcharge.eq.1).and.(ifdipole.eq.0)) then
         if((ifpgh.eq.1)) then
            call g3d_directcp(nd,dim,eps,delta,dmax,
     1           source(1,istart),ns,charge(1,istart),
...
c------------------------------------------------------------------
      ...
      integer ifpwexpform(*), ifpwexpeval(*)
      integer *8 lmptot,istart,itmp,nn,itmp2
      integer ibox,i,nlevstart
c
      istart = 1
