...
c--------------------------------------------------
c     a wrapper for svdpivot
c
      subroutine svdpvwrap(nm,m,n,a,s,u,v,ierr,ncols)
      implicit real*8 (a-h,o-z)
      integer nm, m, n, ierr, lw, ltot
      real*8 a(nm,n), s(n), u(nm,n), v(nm,n)
      real*8, allocatable:: w(:)
      real*8, allocatable:: amat(:,:), umat(:,:), vmat(:,:)

      lw=3*n*m+ 2*(min(m,n))**2 +2*n+2*m+2000
      allocate(w(lw))
      allocate(amat(m,n))
      allocate(umat(m,n))
      allocate(vmat(n,m))
...
c
      eps=1.0d-16
      call svdpivot(ierr,amat,m,n,umat,vmat,s,ncols,eps,
     1     w,lw,ltot)
c
c      write(*,*) m, n, ncols
c
